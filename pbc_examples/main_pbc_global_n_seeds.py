...
parser.add_argument('--n_seeds', type=int, default=1, help='Number of random initializations.')
...
for seed in range(1, args.n_seeds+1):
    ...
    print(f"Training PINN with seed={seed} ...")
    ...
    # TODO: right now we don't recompute losses if checkpoint exists
    # TODO: load model from checkpoint and then evaluate losses
    
    # TODO: add force re-run option 1.0
    # checkpoint_file = f"checkpoints/PINN_{args.system}_beta_{int(beta):02d}_lr_{args.lr}_seed_{args.seed:03d}.pt"
    checkpoint_file = f"checkpoints/PINN_{args.system}_beta_{beta}_lr_{args.lr}_seed_{args.seed:03d}.pt"

    if os.path.exists(checkpoint_file):
        print(f"[+] {checkpoint_file} (already exists!!!)")
        # exit(0)
        continue
    ...
    set_seed(0) # for data processed (keep same across model seeds)
    ...
    # update results
    precision = 6
    df_result = pd.DataFrame(index=[0]).assign(
        beta=args.beta,
        lr=args.lr,
        seed=args.seed,
        error_u_relative=round(error_u_relative, precision),
        error_u_abs=round(error_u_abs, precision),
        error_u_linf=round(error_u_linf, precision),
    ...
    if args.visualize:
        path = f"heatmap_results_global/{args.system}"
    ...
    if args.save_model: # whether or not to save the model
        path = os.path.dirname(checkpoint_file)
        if not os.path.exists(path):
            os.makedirs(path)
...
results_file = f"PINN_{args.system}_beta_{beta}_lr_{args.lr}_n_seeds_{args.n_seeds}.csv"
df_results.round(6).to_csv(results_file, index=False)
